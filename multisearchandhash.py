...
def GetDirectoryTreeContents():
    ...
    try:
        ...
        directoryPath = input("Enter Directory Path i.e. c:/Users or c:\\ >>>: ")
        print("\nUser Entered Directory: ", directoryPath)

        directoryPath = os.path.abspath(directoryPath)
        print(f"→ Scanning: {directoryPath}")
    
        if not os.path.isdir(directoryPath):
            print("Directory not found.")
            raise Exception('Directory not found.')        
        
        pathList: list[str] = []#store all the entries with full path
        dot_counter = 0
       
        for root, dirs, files in os.walk(directoryPath):
            
            for fname in files:
                pathList.append(os.path.join(root, fname))
                dot_counter += 1
                if dot_counter % 1000 == 0:
                    print(".", end="", flush=True)   # progress dot#add entry to list
        
        if not pathList:
            raise Exception("Directory exists but contains no regular files.")

        print()   # newline after any progress dots
        return True, None, pathList, directoryPath  #No Errors, return a list with paths to all paths in sub directories
...
if __name__ == '__main__':
    metadataOutputList = []          # holds [path, size, MAC] for each entry
    print("\nChristopher O'Brien - Version One\n")

    # ── keep asking until we get a usable directory ──────────────────────────
    while True:
        success, errInfo, dirContentNames, root = GetDirectoryTreeContents()

        if success and dirContentNames:      # good path, at least one file
            break

        if not success:
            print(f"✘ {errInfo}")
        else:                                # path was valid but empty
            print("✘ Directory contains no files.")

        print("Please try again, or press Ctrl-C to quit.\n")


    total = len(dirContentNames)
    print(f"\nCollecting metadata for {total:,} files…")

    for idx, entry in enumerate(dirContentNames, 1):
        ok, errInfo, fileSize, macList = GetFileMetaData(entry)
        if ok:
            metadataOutputList.append([entry, fileSize, macList])
        else:
            print(f"Stat failed: {entry}  ({errInfo})")

        if idx % 1000 == 0 or idx == total:
            pct = idx * 100 // total
            print(f"  {pct:3d}%  ({idx:,}/{total:,})", end="\r", flush=True)

    print()        # newline after the last percentage

    # ── build the metadata list for the confirmed directory tree ─────────────
    for entry in dirContentNames:
        success, errInfo, fileSize, macList = GetFileMetaData(entry)
        if success:
            metadataOutputList.append([entry, fileSize, macList])
        else:
            print(f"Failure in metadata retrieval: {entry}  ({errInfo})")
